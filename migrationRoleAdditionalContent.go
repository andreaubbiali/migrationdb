...
func RoleAdditionalContent(dbsorint, dbsircles *sql.DB, timeline map[int64]int64) {
	...
	query := `INSERT INTO roleadditionalcontent (id, start_tl, end_tl, content) VALUES `
	...
	for rows.Next() {
		...
		if end_tl.Valid {
			...
			values = append(values, id, start_tl, end_tl.Int64, content)

		} else {
			// append values to query
			values = append(values, id, start_tl, nil, content)
		}
	}
	// remove last ','
	query = query[:len(query)-1]
